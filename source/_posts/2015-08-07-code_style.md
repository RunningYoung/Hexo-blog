...
As seen here:
![Xcode Page Guide Pref](/attachment/code_style/uncrustify_pref_page_sm.png)


#### Uncrustify 0.61-snapshot

[uncrustify](https://github.com/bengardner/uncrustify)的当前版本有些缺陷。推荐[download my snapshot build](http://mix-pub-dist.s3-website-us-west-1.amazonaws.com/objective-c-style-guide/uncrustify-dev-snapshots/uncrustify-0.61-snapshot.zip)，并替换BBUncrustifyPlugin-Xcode中的uncrustify命令(进入~/Library/Application\ Support/Developer/Shared/Xcode/Plug-ins/Alcatraz.xcplugin替换)。然后使用[".uncrustify-061.cfg"](/attachment/code_style/uncrustify-061.cfg)作为配置文件。
...
尽量使用一个单独的 block 作为接口的最后一个参数。把需要提供的数据和错误信息整合到一个单独 block 中，比分别提供成功和失败的 block 要好。
